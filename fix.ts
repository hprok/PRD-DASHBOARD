...
const firstStartIndex = content.indexOf(startMarker);
const endIndex = content.indexOf(endMarker);

if (firstStartIndex === -1 || endIndex === -1) {
  console.error('Markers not found');
  process.exit(1);
}

// The original panel body is between `{/* Modal Body */}` and the end of the file.
// Let's extract it from the third block.
const thirdStartIndex = content.indexOf(startMarker, content.indexOf(startMarker, firstStartIndex + 1) + 1);

let panelBodyContent = '';
if (thirdStartIndex !== -1) {
    const bodyStartIndex = content.indexOf('{/* Modal Body */}', thirdStartIndex);
    const footerStartIndex = content.indexOf('</motion.div>', bodyStartIndex);
    panelBodyContent = content.substring(bodyStartIndex, footerStartIndex);
} else {
    // Fallback if there's only one block
    const bodyStartIndex = content.indexOf('{/* Modal Body */}', firstStartIndex);
    const footerStartIndex = content.indexOf('</motion.div>', bodyStartIndex);
    panelBodyContent = content.substring(bodyStartIndex, footerStartIndex);
}
...
content = content.substring(0, firstStartIndex) + finalReplacement + content.substring(endIndex);
